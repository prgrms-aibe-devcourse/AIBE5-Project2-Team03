...
export function SignupModal({ open, onOpenChange }: SignupModalProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    passwordConfirm: '',
    username: '',
    roleType: 'MEMBER', // 💡 role -> roleType으로 변경 및 대문자 초기값
    managerName: '',   // 👈 추가
    contactPhone: '',  // 👈 추가
    managerType: 'COMPANY',
  })
  ...
  const trimmedFormData = {
    ...
    roleType: formData.roleType.trim(), // 💡 role -> roleType
  }
  ...
  const isFormValid = Boolean(
      ...
      trimmedFormData.roleType && // 💡 role -> roleType
      agreements.terms &&
      agreements.privacy,
  )
          ...
          <div className="p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-2">
                <Label className="text-sm font-medium text-foreground">회원 유형</Label>
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <label
                      className={`flex items-center justify-center gap-2 rounded-2xl border px-4 py-2 transition ${
                          formData.roleType === 'MEMBER'
                  ...
                  >
                    <input
                        type="radio"
                        name="roleType" // 💡 name 속성 통일
                        value="MEMBER"  // 💡 value 대문자 통일
              ...
              <div className="space-y-2">
                ...
                <Input
                    ...
                    type="text"
                    required
                    placeholder={formData.roleType === 'MANAGER' ? '회사명을 입력해주세요' : '닉네임을 입력하세요'}
              ...
              {/* 기존 이름/담당자 이름 입력란 */}
              <div className="space-y-2">
                <Label htmlFor="modal-username" className="text-sm font-medium text-foreground">
                  {formData.roleType === 'MANAGER' ? '아이디(성함)' : '이름'} <span className="text-destructive">*</span>
                </Label>
                <Input
                    id="modal-username"
                    name="username"
                    type="text"
                    required
                    placeholder={formData.roleType === 'MANAGER' ? '사용하실 아이디를 입력하세요' : '이름을 입력하세요'}
              ...
              {/* 💡 매니저일 때만 나타나는 추가 필드 (담당자명 & 연락처) */}
              {formData.roleType === 'MANAGER' && (
                  <>
                    <div className="space-y-2">
                      <Label htmlFor="modal-managerName" className="text-sm font-medium text-foreground">
                        담당자 실명 <span className="text-destructive">*</span>
                      </Label>
                      <Input
                          id="modal-managerName"
                          name="managerName" // 👈 formData.managerName과 매핑
                          type="text"
                          required
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="modal-contactPhone" className="text-sm font-medium text-foreground">
                        담당자 연락처 <span className="text-destructive">*</span>
                      </Label>
                      <Input
                          id="modal-contactPhone"
                          name="contactPhone" // 👈 formData.contactPhone과 매핑
                          type="tel"
                          required
...
// ... PasswordRule, KakaoIcon, NaverIcon, GoogleIcon 컴포넌트 동일 ...
