...
=======
import {
  Trophy, Calendar, Clock, Users, FileText,
  Building2, CheckCircle, AlertCircle, Star,
  Award, ChevronRight,
} from 'lucide-react'
>>>>>>> origin/kyungsu
...
export default function QuestDetailPage() {
  ...
  const [stats, setStats] = useState({ applicantCount: 0, submissionCount: 0, reviewingCount: 0, selectedCount: 0 })
...
>>>>>>> origin/kyungsu
      try {
        ...
        if (!res.ok) { setQuest(null); return }
        const data: QuestApiResponse = await res.json()
        setQuest(data)

        // 매니저 정보
        try {
          const mRes = await fetch(`http://localhost:8000/api/manager/${data.managerId}`)
          if (mRes.ok) setManager(await mRes.json())
        } catch { /* ignore */ }

        // 같은 의뢰인의 다른 퀘스트
        try {
          const sRes = await fetch(`http://localhost:8000/api/quests/manager/${data.managerId}`)
          if (sRes.ok) {
            const all: QuestApiResponse[] = await sRes.json()
            setSimilarQuests(all.filter((q) => q.id !== data.id && q.status === 'OPEN').slice(0, 3))
          }
        } catch { /* ignore */ }
      } catch {
        setQuest(null)
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const res = await fetch(`http://localhost:8000/api/quests/${questId}/stats`)
...
=======
        // 통계 즉시 갱신
        const sRes = await fetch(`http://localhost:8000/api/quests/${questId}/stats`).catch(() => null)
      ...
      <div className="min-h-screen bg-background">
        ...
        <div className="flex items-center justify-center py-32">
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-sm text-foreground-muted">퀘스트 불러오는 중...</p>
          </div>
        </div>
      </div>
    )
    ...
    return (
      <div className="min-h-screen bg-background">
        ...
        <div className="mx-auto max-w-3xl px-4 py-24 text-center">
          ...
          <p className="mt-2 text-foreground-muted">{questId}번 퀘스트가 존재하지 않습니다.</p>
          <Link href="/quests"><Button className="mt-6">퀘스트 목록으로</Button></Link>
        </div>
      </div>
    )
  }
...
=======
  const techStack: string[] = Array.isArray(formData?.techStack) ? (formData.techStack as string[]) : []
  const difficulty = (formData?.difficulty as string) ?? ''
  const submissionFormats: string[] = Array.isArray(formData?.submissionFormats) ? (formData.submissionFormats as string[]) : []
  ...
  const statusColor = STATUS_COLORS[quest.status] ?? 'bg-gray-100 text-gray-600'
  const difficultyLabel = DIFFICULTY_LABELS[difficulty] ?? difficulty

  const managerDisplayName = manager?.companyName || manager?.nickname || `매니저 #${quest.managerId}`
  const managerType = manager?.managerType === 'COMPANY' ? '기업' : manager?.managerType === 'INDIVIDUAL' ? '개인' : '-'

  const infoItems = [
    { icon: Trophy,    label: '보상',     value: formatReward(quest.rewardAmount), highlight: true },
    { icon: Calendar,  label: '마감',     value: deadlineText,                     highlight: false },
    { icon: Star,      label: '난이도',   value: difficultyLabel || '미설정',      highlight: false },
    { icon: FileText,  label: '제출 형식', value: submissionFormats.length > 0
        ? submissionFormats.map((f) => f === 'github' ? 'GitHub' : f === 'file' ? '파일' : f).join(', ')
        : '미설정',                                                                 highlight: false },
    { icon: Clock,     label: '등록일',   value: new Date(quest.createdAt).toLocaleDateString('ko-KR'), highlight: false },
    { icon: Building2, label: '의뢰 유형', value: managerType,                     highlight: false },
  ]
>>>>>>> origin/kyungsu
      ...
      <div className="border-b border-border bg-surface">
        <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 xl:px-12">
          ...
          <nav className="mb-4 flex items-center gap-1 text-xs text-foreground-muted">
            <Link href="/quests" className="hover:text-primary transition-colors">퀘스트</Link>
...
=======
          <div className="space-y-3">
            <h1 className="text-2xl font-bold text-foreground sm:text-3xl">{quest.title}</h1>
            <div className="flex flex-wrap items-center gap-2 text-sm text-foreground-muted">
              ...
              {techStack.length > 0 && (
                <>
                  <span>·</span>
                  <span>{techStack[0]}{techStack.length > 1 ? ` 외 ${techStack.length - 1}개` : ''}</span>
>>>>>>> origin/kyungsu
                </>
              )}
            </div>
      ...
      <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8 xl:px-12">
          ...
          <div className="space-y-6">
            ...
            {description && (
              <section className="rounded-xl border border-border bg-surface p-6">
                <h2 className="mb-3 text-base font-bold text-foreground">요약</h2>
                <p className="whitespace-pre-line leading-relaxed text-foreground-muted">{description}</p>
              </section>
            )}

            {/* Info Grid */}
            <section className="rounded-xl border border-border bg-surface p-6">
              <h2 className="mb-5 text-base font-bold text-foreground">퀘스트 정보</h2>
            ...
            {techStack.length > 0 && (
              <section className="rounded-xl border border-border bg-surface p-6">
                <h2 className="mb-4 text-base font-bold text-foreground">스킬 & 기술 스택</h2>
                <p className="mb-2 text-xs font-medium text-foreground-muted">필수 스킬</p>
            ...
            {/* Activity */}
            <section className="rounded-xl border border-border bg-surface p-6">
              <h2 className="mb-4 text-base font-bold text-foreground">이 퀘스트의 활동</h2>
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                {[
                  { icon: Users,        label: '지원자',    value: `${stats.applicantCount}명` },
                  { icon: FileText,     label: '제출물',    value: `${stats.submissionCount}건` },
                  { icon: CheckCircle,  label: '검토 중',   value: `${stats.reviewingCount}건` },
                  { icon: Award,        label: '최종 선정', value: stats.selectedCount > 0 ? `${stats.selectedCount}명` : '-' },
                ].map(({ icon: Icon, label, value }) => (
                  <div key={label} className="flex flex-col gap-1 rounded-lg bg-background p-4">
            ...
            {/* About Manager */}
            <section className="rounded-xl border border-border bg-surface p-6">
              <h2 className="mb-4 text-base font-bold text-foreground">의뢰인 정보</h2>
              <div className="flex items-center gap-4 mb-4">
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-primary/10">
                  <Building2 className="h-6 w-6 text-primary" />
                </div>
                <div>
                  <p className="font-semibold text-foreground">{managerDisplayName}</p>
              ...
              <div className="grid grid-cols-2 gap-3 text-xs sm:grid-cols-4">
                {[
                  ...
                  { label: '계정 상태',   value: manager?.approved ? '✓ 인증됨' : '미인증' },
            ...
            <div className="sticky top-24 space-y-4">
              ...
              <div className="rounded-xl border border-border bg-surface p-6 shadow-sm">
                <div className="mb-5 text-center">
                  <p className="text-3xl font-bold text-primary">{formatReward(quest.rewardAmount)}</p>
                  <p className="mt-1 text-xs text-foreground-muted">퀘스트 보상금</p>
                </div>
                <div className="mb-5 grid grid-cols-2 divide-x divide-border border-t border-border pt-4 text-center">
                  <div className="pr-4">
                    <p className="text-sm font-semibold text-foreground">{deadlineText}</p>
                    <p className="text-xs text-foreground-muted">마감</p>
                  </div>
                  <div className="pl-4">
                    <p className={`text-sm font-semibold ${quest.status === 'OPEN' ? 'text-green-600' : 'text-foreground'}`}>{statusLabel}</p>
                ...
                {userRole !== 'MANAGER' && (
                  <div className="space-y-2">
                    {applied ? (
                      <>
                        <Button className="w-full bg-green-600 text-white hover:bg-green-700 cursor-default" disabled>
                          ✓ 참여 중
                        </Button>
                        <Button
                          variant="outline"
                      ...
                      >
                        {applying ? '신청 중...' : quest.status !== 'OPEN' ? '모집 마감' : '퀘스트 참여하기'}
                      </Button>
                    )}
                    {applyError && (
                      <p className="text-center text-xs text-red-500">{applyError}</p>
                    )}
                    <Link href={`/quests/${questId}/submit`} className="block">
                      <Button variant="outline" className="w-full">결과 제출하기</Button>
                    </Link>
                  </div>
                )}
              </div>
              ...
              {submissionFormats.length > 0 && (
                <div className="rounded-xl border border-border bg-surface p-5">
                  <h3 className="mb-3 text-sm font-semibold text-foreground">제출 형식</h3>
                  <ul className="space-y-2">
                    {submissionFormats.map((fmt) => (
                      <li key={fmt} className="flex items-center gap-2 text-sm text-foreground-muted">
                        <CheckCircle className="h-4 w-4 text-green-500 shrink-0" />
                        {fmt === 'github' ? 'GitHub Repository' : fmt === 'file' ? '파일 업로드' : fmt}
                      </li>
                    ))}
                  </ul>
                </div>
              ...
              {difficultyLabel && (
                <div className="rounded-xl border border-border bg-surface p-5">
                  <h3 className="mb-3 text-sm font-semibold text-foreground">난이도</h3>
                  <Badge className={
        ...
        {similarQuests.length > 0 && (
          <section className="mt-14">
            <h2 className="mb-6 text-xl font-bold text-foreground">같은 의뢰인의 다른 퀘스트</h2>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {similarQuests.map((q) => {
                const fd = q.formData as Record<string, unknown>
                const stack: string[] = Array.isArray(fd?.techStack) ? (fd.techStack as string[]) : []
                return (
                  <Link key={q.id} href={`/quests/${q.id}`}>
                    <div className="group rounded-xl border border-border bg-surface p-5 transition-all hover:border-primary/50 hover:shadow-md">
                      <h3 className="line-clamp-2 font-semibold text-foreground group-hover:text-primary">{q.title}</h3>
                      <div className="mt-2 flex items-center justify-between">
                        <span className="text-sm font-bold text-primary">{formatReward(q.rewardAmount)}</span>
                        <span className="text-xs text-foreground-muted">{formatDeadline(q.deadline)}</span>
                      </div>
